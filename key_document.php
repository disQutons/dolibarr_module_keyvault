...
/**
 *  \file       key_document.php
 *  \ingroup    keyvault
 *  \brief      Tab for documents linked to Key
 */
...
$langs->loadLangs(array("keyvault@keyvault", "companies", "other", "mails"));
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->keyvault->multidir_output[$object->entity ? $object->entity : $conf->entity]."/key/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('keyvault', 'key', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("Key")." - ".$langs->trans("Files");
//$title = $object->ref." - ".$langs->trans("Files");
$help_url = '';
//Example $help_url='EN:Module_Third_Parties|FR:Module_Tiers|ES:Empresas';
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-keyvault page-card_document');

// Show tabs
$head = keyPrepareHead($object);

print dol_get_fiche_head($head, 'document', $langs->trans("Key"), -1, $object->picto);
...
$modulepart = 'keyvault';
$param = '&id='.$object->id;
//$relativepathwithnofile='key/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'key/'.dol_sanitizeFileName($object->ref).'/';
