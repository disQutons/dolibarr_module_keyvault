...
require_once __DIR__.'/class/key.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('keyvault', 'key', 'write');
	$permissiontodelete = $user->hasRight('keyvault', 'key', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("keyvault")) {
	accessforbidden('Module keyvault not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	$objectclass = 'Key';
	$objectlabel = 'Key';
	$uploaddir = $conf->keyvault->dir_output;
...
$title = $langs->trans("Keys");
//$help_url = "EN:Module_Key|FR:Module_Key_FR|ES:Módulo_Key";
$help_url = '';
$morejs = array();
$morecss = array();
...
foreach ($search as $key => $val) {
	...
	if ($key == 'rights_group' && $val != '') {
		$sql .= " AND (FIND_IN_SET(".((int) $val).", t.rights_group) OR EXISTS (SELECT 1 FROM ".$db->prefix()."usergroup ug WHERE ug.nom LIKE '%".$db->escape($val)."%' AND FIND_IN_SET(ug.rowid, t.rights_group)))";
		continue;
	}
	if ($key == 'rights_user' && $val != '') {
		// Allow search by id, firstname, lastname, "firstname lastname" or "lastname firstname"
		$val_trim = trim($val);
		$val_esc = $db->escape($val_trim);
		$tokens = preg_split('/\s+/', $val_trim);

		$sql .= " AND (FIND_IN_SET(".((int) $val).", t.rights_user) OR EXISTS (SELECT 1 FROM ".$db->prefix()."user u WHERE (";

		// Match concatenated firstname + ' ' + lastname or reversed
		$sql .= "CONCAT(u.firstname, ' ', u.lastname) LIKE '%".$val_esc."%' OR CONCAT(u.lastname, ' ', u.firstname) LIKE '%".$val_esc."%'";

		// If user provided multiple tokens, require each token to match either firstname or lastname (in any order)
		if (is_array($tokens) && count($tokens) > 1) {
			$parts = array();
			foreach ($tokens as $tkn) {
				$t = $db->escape($tkn);
				$parts[] = "(u.firstname LIKE '%".$t."%' OR u.lastname LIKE '%".$t."%')";
			}
			$sql .= " OR (".implode(' AND ', $parts).")";
		}

		// Also fallback to single-name match
		$sql .= " OR u.firstname LIKE '%".$val_esc."%' OR u.lastname LIKE '%".$val_esc."%') AND FIND_IN_SET(u.rowid, t.rights_user)))";
		continue;
	}
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/keyvault/key_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-keyvault page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/keyvault/key_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendKeyRef";
$modelmail = "key";
$objecttmp = new Key($db);
...
// Prepare caches for name resolution
$cacheUsers = array();
$cacheGroups = array();
...
while ($i < $imaxinloop) {
	...
	// Replace rights_user ids by names for display
	if (!empty($object->rights_user)) {
		$ids = preg_split('/\s*,\s*/', $object->rights_user, -1, PREG_SPLIT_NO_EMPTY);
		$names = array();
		foreach ($ids as $uid) {
			$uid = (int) $uid;
			if ($uid <= 0) continue;
			if (!array_key_exists($uid, $cacheUsers)) {
				$u = new User($db);
				if ($u->fetch($uid) > 0) {
					$cacheUsers[$uid] = $u->getFullName($langs);
				} else {
					$cacheUsers[$uid] = null;
				}
			}
			if (!empty($cacheUsers[$uid])) $names[] = $cacheUsers[$uid];
		}
		$object->rights_user = implode(', ', $names);
	}

	// Replace rights_group ids by group names for display
	if (!empty($object->rights_group)) {
		$gids = preg_split('/\s*,\s*/', $object->rights_group, -1, PREG_SPLIT_NO_EMPTY);
		$gnames = array();
		foreach ($gids as $gid) {
			$gid = (int) $gid;
			if ($gid <= 0) continue;
			if (!array_key_exists($gid, $cacheGroups)) {
				$gg = new UserGroup($db);
				if ($gg->fetch($gid) > 0) {
					$cacheGroups[$gid] = $gg->nom;
				} else {
					$cacheGroups[$gid] = null;
				}
			}
			if (!empty($cacheGroups[$gid])) $gnames[] = $cacheGroups[$gid];
		}
		$object->rights_group = implode(', ', $gnames);
	}
